...
from authentik.lib.models import DomainlessURLValidator, SimpleThroughModel
...
class SAMLSPPropertyMapping(SimpleThroughModel):
    """Explicit through model for SAMLSP.property_mappings."""

    samlsp = models.ForeignKey("authentik_providers_saml.SAMLSP", on_delete=models.CASCADE)
    samlpropertymapping = models.ForeignKey(
        "authentik_providers_saml.SAMLPropertyMapping", on_delete=models.CASCADE
    )

    class Meta:
        unique_together = (("samlsp", "samlpropertymapping"),)
        verbose_name = _("SAML SP Property Mapping")
        verbose_name_plural = _("SAML SP Property Mappings")

    def __str__(self) -> str:
        return (
            f"SAMLSPPropertyMapping for SP {self.samlsp_id} "
            f"and PropertyMapping {self.samlpropertymapping_id}."
        )
