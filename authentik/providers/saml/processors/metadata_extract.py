...
def iter_entity_descriptors(root: etree._Element) -> Iterator[etree._Element]:
    """Yield EntityDescriptor nodes from single or aggregate metadata XML."""
    if root.tag == f"{{{NS_SAML_METADATA}}}EntityDescriptor":
        yield root
        return
    if root.tag == f"{{{NS_SAML_METADATA}}}EntitiesDescriptor":
        yield from root.xpath(".//md:EntityDescriptor", namespaces=NS_MAP)
        return
    raise ValueError("Root element must be EntityDescriptor or EntitiesDescriptor")


def iter_sp_entity_descriptors(raw_xml: str) -> Iterator[etree._Element]:
    """Yield SP EntityDescriptor nodes from metadata XML text."""
    root = lxml_from_string(raw_xml)
    for entity in iter_entity_descriptors(root):
        if has_sp_descriptor(entity):
            yield entity


def iter_idp_entity_descriptors(raw_xml: str) -> Iterator[etree._Element]:
    """Yield IdP EntityDescriptor nodes from metadata XML text."""
    root = lxml_from_string(raw_xml)
    for entity in iter_entity_descriptors(root):
        if has_idp_descriptor(entity):
            yield entity


def has_sp_descriptor(entity: etree._Element) -> bool:
    """Return True when EntityDescriptor contains SPSSODescriptor."""
    return bool(entity.xpath("./md:SPSSODescriptor", namespaces=NS_MAP))


def has_idp_descriptor(entity: etree._Element) -> bool:
    """Return True when EntityDescriptor contains IDPSSODescriptor."""
...
def normalize_binding_uri_to_token(binding_uri: str) -> str:
    ...
    if binding_uri == SAML_BINDING_POST:
        return "post"
    if binding_uri == SAML_BINDING_REDIRECT:
        return "redirect"
    return binding_uri
...
def _extract_all_candidates(desc: etree._Element, element_xpath: str) -> list[dict[str, Any]]:
    ...
    for n in desc.xpath(element_xpath, namespaces=NS_MAP):
        ...
        is_default = (n.attrib.get("isDefault") or "").strip().lower() == "true"
        results.append(
            {"url": url, "binding": binding, "index": index, "is_default": is_default}
        )
...
def extract_entity_display_names(entity: etree._Element) -> list[dict[str, str]]:
    """Extract EntityDescriptor mdui:DisplayName values with xml:lang."""
    ...
    for xp in xpaths:
        for node in entity.xpath(xp, namespaces=NS_MAP):
            text = (node.text or "").strip()
            if not text:
                continue
            lang = node.attrib.get("{http://www.w3.org/XML/1998/namespace}lang", "").strip().lower()
            key = (text, lang)
            if key in seen:
                continue
            seen.add(key)
    ...
    *,
    ...
    for s in services:
        ...
        if binding not in allowed_bindings:
            continue
        norm.append({"binding": binding, "url": url, "is_default": bool(s.get("is_default"))})

    if not norm:
        return None

    def _clean(s: dict[str, Any]) -> dict[str, str]:
        return {"binding": s["binding"], "url": s["url"]}

    # An endpoint explicitly marked isDefault (IndexedEndpointType) wins over
    # document/index order; fall back to the full set otherwise.
    for pool in ([s for s in norm if s["is_default"]], norm):
        if not pool:
            continue
        for b in prefer_order:
            for s in pool:
                if s["binding"] == b:
                    return _clean(s)
        return _clean(pool[0])
    return _clean(norm[0])
