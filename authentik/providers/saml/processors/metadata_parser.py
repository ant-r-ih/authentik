...
class ServiceProviderMetadataParser:
    ...
    def _parse_entity(self, entity: etree._Element) -> ServiceProviderMetadata:
        ...
        if not runtime.get("acs_url"):
            raise ValueError(
                "Metadata has no AssertionConsumerService with a supported binding"
            )
    ...
    def iter_entities(self, raw_xml: str) -> Iterator[ServiceProviderMetadata]:
        ...
        Aggregate (federation) metadata may hold entities authentik cannot import
        (e.g. no ACS with a supported binding). Such entities are skipped rather
        than aborting the whole walk; document-level errors still propagate."""
        for entity in mx.iter_sp_entity_descriptors(raw_xml):
            try:
                yield self._parse_entity(entity)
            except ValueError as exc:
                LOGGER.warning(
                    "Skipping SP entity that cannot be parsed",
    ...
    def parse(self, raw_xml: str) -> ServiceProviderMetadata:
        ...
        descriptors = list(mx.iter_sp_entity_descriptors(raw_xml))
        if not descriptors:
        ...
        return self._parse_entity(descriptors[0])
...
class IdentityProviderMetadataParser:
    ...
    def iter_entities(self, raw_xml: str) -> Iterator[IdentityProviderMetadata]:
        ...
        Aggregate (federation) metadata may hold entities that cannot be parsed
        (e.g. invalid certificate or signature). Such entities are skipped rather
        than aborting the whole walk; document-level errors still propagate."""
        for entity in mx.iter_idp_entity_descriptors(raw_xml):
            try:
                ...
                LOGGER.warning(
                    "Skipping IdP entity that cannot be parsed",
                    entity_id=entity.attrib.get("entityID"),
                    error=str(exc),
                )
    ...
    def parse(self, raw_xml: str) -> IdentityProviderMetadata:
        ...
        descriptors = list(mx.iter_idp_entity_descriptors(raw_xml))
        if not descriptors:
