...
from authentik.core.tests.utils import create_test_flow, create_test_cert
...
class TestImportIDPKeyFreezeAndMode(TestCase):
    ...
    def test_inherit_allows_rotation(self):
        ...
        self.assertEqual(idp.verification_kp_mode, SAMLIDPKeyOverrideMode.SET)

        # Make it "inherit" so importer is allowed to change it
        idp.verification_kp_mode = SAMLIDPKeyOverrideMode.INHERIT
        idp.verification_kp = None
        idp.save(update_fields=["verification_kp_mode", "verification_kp"])
        ...
        self.assertEqual(idp2.verification_kp_mode, SAMLIDPKeyOverrideMode.SET)
    ...
    def test_mode_set_blocks_rotation(self):
        idp, _ = import_idp_from_entity_descriptor(source=self.source, entity=self.ent1, enabled=True)
        idp.verification_kp_mode = SAMLIDPKeyOverrideMode.SET
        idp.save(update_fields=["verification_kp_mode"])
        old_kp = idp.verification_kp_id
    ...
    def test_mode_none_blocks_rotation(self):
        idp, _ = import_idp_from_entity_descriptor(source=self.source, entity=self.ent1, enabled=True)
        idp.verification_kp_mode = SAMLIDPKeyOverrideMode.NONE
        idp.save(update_fields=["verification_kp_mode"])
        old_kp = idp.verification_kp_id
    ...
    def test_no_cert_sets_inherit_when_overwrite_allowed(self):
        ...
        self.assertEqual(idp.verification_kp_mode, SAMLIDPKeyOverrideMode.INHERIT)
